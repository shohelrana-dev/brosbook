...
export default function SearchUserList({
	...
	return (
		<div className={cn('min-w-72 max-h-130 p-3 overflow-y-auto', wrapperClassname)}>
			...
			{users && users.length > 0
				? users.map(user => (
						...
						>
							<UserItem
								user={user}
								className={cn('mb-0', handleUserClick && 'pointer-events-none')}
			...
			{!searchText ? (
				<p className='text-gray-800 p-3 pb-4'>Try searching for people, topics.</p>
			) : null}

			{isSuccess && users.length < 1 ? (
				<p className='text-gray-800 p-3 pb-4'>No results found.</p>
			) : null}
		</div>
	)
}
